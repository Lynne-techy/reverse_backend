...
4. **streak/잔디의 "하루" 경계 타임존**: 글로벌 서비스라 UTC 고정은 사용자 체감과 어긋날 수 있음. 사용자별 로컬 타임존 저장/반영 여부 결정 필요 (`daily_verses`는 클라이언트 제공 날짜로 이미 해결 — 조작 방지가 필요한 streak에는 같은 방식을 그대로 쓸 수 없음).
