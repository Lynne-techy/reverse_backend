...
- [ ] ④-2 `writing` 모듈(업로드~통과 stub), ④-3 `stats` 모듈(streak/잔디) → ⑤ Dockerfile/.env.example 배포 인계
- [ ] streak/잔디의 타임존 경계는 별도 열린 질문(`docs/DATABASE.md` §5-4) — 조작 방지가 필요해 daily_verses와 같은 방식을 못 씀
- [ ] (이후) emotion_tags, verse_emotion_tags, quests, user_quests

## 최근 세션
- 2026-07-07: `verse` 모듈 구현+검증 완료(위 참조). 브랜치를 `feat/w2-verse-writing-streak`로 분리. API 문서화를 위해 `@nestjs/swagger` 도입 예정.
